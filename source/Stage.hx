				...
				case 'downtown':
					{
						...
						var windowTemp:FlxSprite = new FlxSprite(-768,-592).loadGraphic(Paths.image('downtown/Win0','weekCuz'));
						...
						skybox.scrollFactor.set(0.45, 0.45);
						skybox.active = false;
						toAdd.push(skybox);
						//Buildings
						buildings.setGraphicSize(Std.int(buildings.width * 1));
						buildings.scrollFactor.set(0.5, 1);
						...
						if (FlxG.save.data.distractions)
						{
                            ...
                            toAdd.push(downtownWindows);
						} 
						else {
							// window temp (actually just used if no distractions)
							windowTemp.setGraphicSize(Std.int(windowTemp.width * 1));
							windowTemp.scrollFactor.set(0.5, 1);
							windowTemp.active = false;
							toAdd.push(windowTemp);
						}

						for (i in 0...5)
						{
							var windows:FlxSprite = new FlxSprite(-768,-592).loadGraphic(Paths.image('downtown/Win' + i, 'weekCuz'));
							windows.scrollFactor.set(0.5, 1);
						...
						// these should be infront of the players
						//Spotlight
						streetlight.setGraphicSize(Std.int(streetlight.width * 1));
						streetlight.scrollFactor.set(1, 1);
						...
						//test
						/*
						var testBg:FlxSprite = new FlxSprite(-768,-592).loadGraphic(Paths.image('downtown/backgroundfull','weekCuz'));
						testBg.antialiasing = true;
						testBg.setGraphicSize(Std.int(testBg.width * 1));
						testBg.scrollFactor.set(1, 1);
						testBg.active = false;
						testBg.updateHitbox();
						toAdd.push(testBg);
						*/
