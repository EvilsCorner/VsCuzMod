...
class PlayState extends MusicBeatState
{
	...
	// dumb variables for stabbing people!
	...
	override public function create()
	{
		...
		/// so far it seems kade did stages better
		/// bc you had easy access to layering and it was all segmented into one file
		/// where as here, we have it in playstate...
		/// although BGSprite makes things cleaner,
		/// BGSprite itself assumes libraries will just work based on the stage/week
		/// so far, that hasn't worked for adding new ones!
		switch (curStage)
		{
			case 'stage': //Week 1
			...
			case 'downtown':
				...
				var skybox:BGSprite = new BGSprite('downtown/Skybox2', px, py, 0.45, 0.45);
				add(skybox);

				var buildings:BGSprite = new BGSprite('downtown/Buildings', px, py, 0.5, 1);
				add(buildings);

				//var downtownWindows = new FlxTypedGroup<FlxSprite>();
				...
				//front layers
				streetlight = new BGSprite('downtown/Streetlight', px+offSet, py, 1, 1);
				//add(streetlight);
				frontBushes = new BGSprite('downtown/Front-Bushes', px+offSet, py, 1.1, 1.1);
				//add(frontBushes);
				shadeOverlay = new BGSprite('downtown/Overlay', px+offSet, py, 1, 1);
				//add(shadeOverlay);
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					if(Paths.formatToSongPath(SONG.song) == 'equivocation')
					{
						Unlockables.gallery = true;
						Unlockables.song4 = true;
						Unlockables.saveUnlocks();
					}
